...
(function() {
  var checkRows, fillR, fillRE, fillREL, personDataGet, personDataUX, personID, picChange, prevPic, relative, relativeData, requester, responsable, responsableDataGet, responsableDataUX, responsableID, setup, spinner, tabsEnabled, uploadPicture;
  ...
  $(document).on("turbolinks:load", function() {
    ...
    $('a#edit-person').on('click', function() {
      personDataUX('enable');
      $('a#update-person').fadeToggle('slow');
      $('a#edit-person').fadeToggle('slow');
      return $('#person_picture').show();
    });
    $('a#update-person').on('click', function() {
      requester = [];
      personDataGet();
      spinner('ON');
      return $.ajax({
    ...
    $('a#save-responsable').on('click', function() {
      var theURL;
      spinner('ON');
      responsableDataGet();
    ...
    $('a#edit-responsable').on('click', function() {
      $('a#update-responsable').show();
      $('a#edit-responsable').fadeToggle('slow');
      return responsableDataUX('enable');
    });
    $('a#update-responsable').on('click', function() {
      responsable = [];
      responsableDataGet();
      spinner('ON');
      return $.ajax({
        ...
        success: function(data) {
          ...
          spinner('OFF');
          $('a#edit-responsable').fadeToggle('slow');
          return $('a#update-responsable').fadeToggle('slow');
        },
        error: function(data) {
          alert(data);
          return spinner('OFF');
    ...
    $('a#addRelative').on('click', function() {
      var $age, $civil, $gender, $name, $occupation, $relationship, $salary, $scolar, items;
      items = [];
      $name = $('#relative_name');
      $age = $('#relative_age');
      ...
      if ($name.val() === '') {
        return alert('debe proporcionar un nombre');
      } else {
        if ($age.val() === '' || $age.val() === null) {
          return alert('debe proporcionar la edad');
        } else {
          if ($gender.val() === '') {
            return alert('debe proporcionar el sexo');
          } else {
            if ($civil.val() === '') {
              return alert('debe proporcionar el estado civil');
            } else {
              if ($relationship.val() === '') {
                return alert('debe proporcionar el parentesco');
              } else {
                if ($occupation.val() === '') {
                  return alert('debe proporcionar la ocupacion');
                } else {
                  if ($scolar.val() === '') {
                    return alert('debe proporcionar la escolaridad');
                  } else {
                    $('#relationshipTable tbody').append('<tr><td>' + $name.val() + '</td> <td>' + $relationship.val() + '</td> <td>' + $age.val() + '</td> <td>' + $gender.val() + '</td> <td>' + $scolar.val() + '</td><td>' + $civil.val() + '</td> <td>' + $occupation.val() + '</td><td><a class="btn btn-sm btn-default edrelative"><span class="glyphicon glyphicon-pencil"></span></a>' + '</td><td><a class="btn btn-sm btn-danger"><span class="glyphicon glyphicon-trash delrelative"></span></a></td></tr>');
                    ...
                    return $('.delrelative').on('click', function() {
                      var $row;
                      $row = $(this).closest('tr');
                      $row.remove();
                      return checkRows();
    ...
    $('#save_relatives').on('click', function() {
      relative = [];
      $('#relationshipTable tbody tr').each(function() {
        relativeData = [];
        $(this).find('td').each(function() {
          if ($(this).text() !== '') {
            return relativeData.push($(this).text());
          }
        });
        return relative.push(relativeData);
      });
      spinner('ON');
      return $.ajax({
        type: 'POST',
        ...
        success: function(data) {
          ...
          return window.location.replace('/people/' + personID);
        },
        error: function(data) {
          alert(data);
          return spinner('OFF');
    ...
    return $('#person_picture').change(function() {
      prevPic(this);
      $('#img_prev').show();
      picChange = true;
      if (picChange === true) {
        if ($('h1').text() === 'Edicion Solicitante') {
          return $('#actual_picture').fadeToggle('slow');
        }
      }
    });
  });
  ...
  setup = function() {
    if ($('h1').text() === 'Edicion Solicitante') {
      ...
      $('#addRelative').show();
      $('#img_prev').hide();
      return $('#actual_picture').hide();
    }
  };

  personDataGet = function() {
    ...
    return requester.push($('#person_workplace').val());
  };
  ...
  checkRows = function() {
    ...
    if (rows === 0) {
      return $('#save_relatives').attr('disabled', true);
    }
  };

  tabsEnabled = function(opt) {
    if (opt) {
      ...
      return $('.nav li#relative').fadeToggle('slow');
    } else {
      ...
      $('.nav li#relative').addClass('disabled');
      return $('.nav li#relative').find('a').removeAttr("data-toggle");
    }
  };
  ...
  personDataUX = function(opt) {
    if (opt === 'disable') {
      ...
      return $('#person_workplace').attr('disabled', true);
    } else {
      ...
      return $('#person_workplace').attr('disabled', false);
    }
  };

  responsableDataUX = function(opt) {
    if (opt === 'disable') {
      ...
      return $('#responsable_relationship').attr('disabled', true);
    } else {
      ...
      return $('#responsable_relationship').attr('disabled', false);
    }
  };

  responsableDataGet = function() {
    ...
    return responsable.push($('#responsable_relationship').val());
  };
  ...
  fillR = function() {
    ...
    return $('#person_workplace').val('UNDERGROUND');
  };
  ...
  fillRE = function() {
    ...
    return $('#responsable_relationship').val('UHMUCULO');
  };
  ...
  fillREL = function() {
    ...
    return $('#relative_relationship').val('PATHER');
  };

  prevPic = function(input) {
    var reader;
    if (input.files && input.files[0]) {
      reader = new FileReader;
      reader.onload = function(e) {
        return $('#img_prev').attr('src', e.target.result);
      };
...
(function() {
  $(document).on("turbolinks:load", function() {
    ...
    if ($('h1').text().match(pattern)) {
      return $('#addRelative').hide();
    }
  });

}).call(this);
(function() {
  var bene, caseUIEnabled, expe, fillCase, fillExp, getBenefitsIDs, getExpeditureIDs, getServicesIDs, has, oexp, personID, requ, serv, setup, spinner, tabsEnabled, validCaseData, validExpensivesData, validLivingData, vita;
  ...
  $(document).on('turbolinks:load', function() {
      ...
      if ($('input#name').val() !== '') {
        ...
        return $.ajax({
          ...
          success: function(response) {
            ...
            return $('.selNamebtn').on('click', function() {
              var $row;
              $row = $(this).closest('tr');
              personID = $row.find('td').first().text();
              $('#dudename').html('<br><div class="alert alert-dismissible alert-info">' + '<strong>Solicitud para: </strong><a href="' + '/people/' + personID + '" class="alert-link">' + $row.find('td').eq(1).text() + '</a></div>');
        ...
        return alert('proporcionar el nombre a buscar');
      }
    });
    $('#editPpl').on('click', function() {
      $('input#name').val('');
      $('input#name').attr('disabled', false);
      $('#editPpl').fadeToggle('slow');
      return $('#seachPpl').fadeToggle('slow');
    });
    $('#addExpediture').on('click', function() {
      ...
      return $.ajax({
        ...
        success: function(response) {
          ...
          return $('#modal-window2').modal('show');
        },
        error: function(response) {
          return alert(response);
        }
    ...
    $('#addService').on('click', function() {
      ...
      return $.ajax({
        ...
        success: function(response) {
          ...
          return $('#modal-window3').modal('show');
        },
        error: function(response) {
          return alert(response);
        }
    ...
    $('#saveService').on('click', function() {
      if ($('#other_service_name').val() === '') {
        return alert('proporcionar un nombre de servicio');
      }
    });
    ...
    $('#living_place_kind').on('change', function() {
    ...
    $('#living_place_wall_material').on('change', function() {
    ...
    $('#living_place_roof_material').on('change', function() {
    ...
    $('#living_place_floor_material').on('change', function() {
    ...
    $('#modal-window').on('hidden.bs.modal', function() {
      return spinner('OFF', $('#spinnerContainer4case'));
    });
    $('#modal-window2').on('show.bs.modal', function() {
      $('#new-benefit-name').hide();
      $('#label-new-benefit').hide();
      $('#save-benefit-name').hide();
      $('#benefit_name').on('change', function() {
        if ($(this).val() === 'OTRO') {
          ...
          return $('#benefit_name').attr('disabled', true);
        }
      });
      return $('#save-benefit-name').on('click', function() {
        var lastValue;
        if ($('#new-benefit-name').val() === '') {
          ...
          return $('#benefit_name').attr('disabled', false);
        }
      });
    });
    $('#modal-window2').on('hidden.bs.modal', function() {
      return spinner('OFF', $('#spinnerContainer4exp'));
    });
    $('#modal-window3').on('hidden.bs.modal', function() {
      ...
      if ($('h4').attr('id') === 'newKind') {
        if ($('#textinput').val() !== '') {
          ...
          return $('#living_place_kind').val($('#textinput').val());
        } else {
          return alert('debe proporcionar un tipo');
        }
      } else {
        if ($('h4').attr('id') === 'newWall') {
          if ($('#wall').val() !== '') {
            $('#living_place_wall_material').append('<option value="' + $('#wall').val() + '">' + $('#wall').val() + '</option>');
            return $('#living_place_wall_material').val($('#wall').val());
          } else {
            return alert('debe proporcionar un material');
          }
        } else {
          if ($('h4').attr('id') === 'newRoof') {
            if ($('#roof').val() !== '') {
              $('#living_place_roof_material').append('<option value="' + $('#roof').val() + '">' + $('#roof').val() + '</option>');
              return $('#living_place_roof_material').val($('#roof').val());
            } else {
              return alert('debe proporcionar un material');
            }
          } else {
            if ($('h4').attr('id') === 'newFloor') {
              if ($('#floor').val() !== '') {
                ...
                return $('#living_place_floor_material').val($('#floor').val());
              } else {
                return alert('debe proporcionar un material');
              }
            }
    ...
    return $('#modal-window3').on('show.bs.modal', function() {
      return $('.badBoy').keypress(function(event) {
        if (event.keyCode === 13) {
  ...
  tabsEnabled = function(opt) {
    if (opt) {
      ...
      return $('.nav li#live').fadeToggle('slow');
    } else {
      $('.nav li#expedit').hide();
      $('.nav li#live').hide();
      ...
      return $('.nav li#live').find('a').removeAttr("data-toggle");
    }
  };

  caseUIEnabled = function(opt) {
    if (opt) {
      ...
      return $('#request_notes').attr('disabled', false);
    } else {
      ...
      return $('#request_notes').attr('disabled', true);
    }
  };
  ...
  setup = function() {
    tabsEnabled(false);
    $('#editPpl').hide();
    return caseUIEnabled(false);
  };

  spinner = function(opt, container) {
    if (opt === 'ON') {
      container.spin({
        lines: 12,
        length: 7,
  ...
  fillCase = function() {
    ...
    return $('#request_notes').val('HEs LYNING DONT TRUST HIM');
  };
  ...
  fillExp = function() {
    ...
    return $('#expediture_education').val('100');
  };

  validCaseData = function() {
    var requestDataFlag;
    requ = [];
    ...
    if (requestDataFlag) {
      alert('Faltaron datos de llenar en Caso');
    }
    if (!requestDataFlag) {
      return true;
    }
  ...
  validExpensivesData = function() {
    var requestExpensivesDataFlag;
    expe = [];
    requestExpensivesDataFlag = false;
    if ($('#expediture_feeding').val() !== '') {
    ...
    if (requestExpensivesDataFlag) {
      alert('faltan datos de capturar en Gastos');
    }
  ...
  validLivingData = function() {
    var requestLivigDataFlag;
    vita = [];
    ...
    if ($('#living_place_number_of_rooms').val() !== '') {
    ...
    if ($('#living_place_has_beedroom').is(':checked')) {
      has.bedroom = true;
    }
    if ($('#living_place_has_kitchen').is(':checked')) {
      has.kitchen = true;
    }
    if ($('#living_place_has_dinningroom').is(':checked')) {
      has.dinningroom = true;
    }
    if ($('#living_place_has_bathroom').is(':checked')) {
      has.bathroom = true;
    }
    if (requestLivigDataFlag) {
      alert('datos faltantes de capturar en Vivienda');
    }
  ...
  getExpeditureIDs = function() {
    oexp = [];
    return $('#otherExpedituresTable tbody tr').each(function() {
      return oexp.push($(this).attr('id'));
    });
  };

  getBenefitsIDs = function() {
    bene = [];
    return $('#benefitsTable tbody tr').each(function() {
      return bene.push($(this).attr('id'));
    });
  };

  getServicesIDs = function() {
    serv = [];
    return $('#servicesTable tbody tr').each(function() {
      return serv.push($(this).attr('id'));
    });
  };
